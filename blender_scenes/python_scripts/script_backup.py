...
os.mkdir(scene_path_yocto)
os.mkdir(scene_path_yocto+"/shapes/")
textures_path=scene_path_yocto+"/textures/"
os.mkdir(textures_path)
os.mkdir(scene_path_dtam)
...
for obj in bpy.data.objects:
    ...
    if obj.type=="CAMERA":
        ...
        lens=bpy.data.cameras[name].lens/1000
        width=bpy.data.cameras[name].sensor_width/1000

        data['cameras'][name]={}
        data['cameras'][name]['aspect']=aspect
        data['cameras'][name]['width']=width
        data['cameras'][name]['resolution']=resolution_x
        ...
        if active_material is not None and 'Image Texture' in obj.active_material.node_tree.nodes:
            ...
            name_tex=name_tex[:-4]
            src=bpy.path.abspath("//bunny_scene.blend")
            src_arr=src.split('/')
            src_arr=src_arr[:-1]
            src="/".join(src_arr)
            src=src+filepath[1:]
            dst=textures_path
            copy(src, dst)
            data['materials'][name]['color_tex']=name_tex

with open(scene_path_yocto+"/"+scene_name+".json", 'w') as outfile:
    json.dump(data, outfile)



#############################################################################
#		generate json for dtam
#############################################################################
