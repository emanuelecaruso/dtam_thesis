...
int main (int argc, char * argv[]) {
  ...
  Camera* camera_m2 = new Camera("Camera_m1",lens,aspect,film,resolution,max_depth,frame_camera_wrt_world_m2,frame_world_wrt_camera_m2);
  ...
  float left_bound=-object_depth/2-(0.1*object_depth);
  float right_bound=(object_depth/2)+(0.1*object_depth);
  int density=8000;
  ...
  dtam->getDepthMap(camera_vector);

  t_end_projection=getTime();
  cerr << "discrete cost volume computation took: " << (t_end_projection-t_start_projection) << " ms" << endl;

  Eigen::Vector2i pixel_coords_r(36,61);


  // Eigen::Vector2i pixel_coords_r(0,61);
  // Eigen::Vector2i pixel_coords_r(10,90);
  // Eigen::Vector2i pixel_coords_r(63,38);
  // Eigen::Vector2i pixel_coords_r(48,60);
  // Eigen::Vector2i pixel_coords_r(63,40);
  // Eigen::Vector2i pixel_coords_r(95,60);
  //
  // std::vector<std::tuple<float,int,Eigen::Vector2f>> costInvdepthUvTuple;
  // dtam->collectCostsInEpipolarLine(pixel_coords_r, camera_r, camera_m1, costInvdepthUvTuple);
  // std::vector<std::vector<std::tuple<float,int,Eigen::Vector2f>>> allCosts;
  // std::vector<std::tuple<float,int,Eigen::Vector2f>> totalCosts;
  // allCosts.push_back(costInvdepthUvTuple);
  // dtam->getTotalCosts(allCosts, totalCosts);
  // int index = dtam->getIndexOfMinimumCost(totalCosts);
  // float depth_m = 1.0/std::get<0>(totalCosts[index]);
  // // int cost = std::get<1>(totalCosts[index]);
  // // Eigen::Vector2f uv_m = std::get<2>(totalCosts[index]);
  // // Eigen::Vector3f p;
  // // camera_vector[1]->pointAtDepth(uv_m, depth_m, p);
  // // Eigen::Vector2f uv_r;
  // // float depth_r;
  // // camera_vector[0]->projectPoint(p, uv_r, depth_r );
  //
  // float depth_r_gt;
  // depth_map_gt->evalPixel(pixel_coords_r,depth_r_gt);
  //
  // std::cout << "depth_r_gt: " << depth_r_gt*max_depth << std::endl;
  // std::cout << "invdepth_r_estimated: " << std::get<0>(totalCosts[index]) << std::endl;
  // std::cout << "depth_r_estimated: " << depth_m << std::endl;
  ...
  // Eigen::Vector3f p;
  // camera_r->pointAtDepth(uv_r, depth, p);
  // // camera_r->pointAtDepth(uv_r, 0.8, p);
  // Eigen::Vector2f uv_m;
  // float p_cam_z;
  // camera_m1->projectPoint(p, uv_m, p_cam_z );
  // Eigen::Vector2i pixel_coords_m;
  // camera_m1->uv2pixelCoords( uv_m, pixel_coords_m);
  //
  // std::cout << "pixel_coords_m: " << pixel_coords_m << std::endl;

  // cv::Vec3b clr(255,0,0);
  // camera_r->image_rgb_->setPixel(pixel_coords_r, clr);
  // camera_r->depth_map_->setPixel(pixel_coords_r, 1.0);
  // camera_m1->image_rgb_->setPixel(pixel_coords_m, clr);
  //
  //
  //
  for (Camera* camera : camera_vector){
