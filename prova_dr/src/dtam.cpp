...
void Dtam::updateDepthMap(int index_m, bool check){
  ...
  for (int j = 0; j<rows*cols; j++){
    ...
    for (int i=0; i<num_interpolations_; i++){
      ...
      cost_matrix_->evalPixel(row,col+cols*i,cost_i);
      n_valid_proj_matrix_->evalPixel(row,col+cols*i,num_valid_projections);
      ...
      if (cost_i<999999){
        ...
        cost_matrix_->setPixel(row,col+cols*i,cost_i);
        n_valid_proj_matrix_->setPixel(row,col+cols*i,(num_valid_projections+1));
          ...
          for(int i = bi;i<ei;i++)
          {
            // inner loop
            {
              ...
              for (int j=0; j<num_interpolations_; j++){
                ...
                cost_matrix_->evalPixel(row,col+cols*j,cost_i);
                n_valid_proj_matrix_->evalPixel(row,col+cols*j,num_valid_projections);
                ...
                if (check){
                  ...
                  if (j==0)
                    camera_m->image_rgb_->setPixel(pixel_current,red);
                  else if(j==num_interpolations_-1)
                    camera_m->image_rgb_->setPixel(pixel_current,blue);
                  else
                    camera_m->image_rgb_->setPixel(pixel_current,clr);
                }
                ...
                if (cost_i<999999){
                  ...
                  cost_matrix_->setPixel(row,col+cols*j,cost_i);
                  n_valid_proj_matrix_->setPixel(row,col+cols*j,(num_valid_projections+1));
